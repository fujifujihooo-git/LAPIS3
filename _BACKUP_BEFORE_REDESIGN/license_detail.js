document.addEventListener('DOMContentLoaded', async () => {
    ...
    function selectCustomer(cust) {
        ...
        customerNameDisplay.textContent = cust.customer_name;
    ...
    async function handleSave() {
        ...
        try {
            if (currentId === 'new') {
                const nextId = licenses.length > 0 ? Math.max(...licenses.map(l => l.license_id)) + 1 : 1;
                data.license_id = nextId;
                data.created_date = now;
                await saveToFirestore('customer_licenses', `lic_${nextId}`, data);
                if (changedBy.value) {
                    const hId = Date.now();
                ...
                await saveToFirestore('customer_licenses', `lic_${currentId}`, data);
            }
    ...
    async function handleDelete() {
        ...
        try {
            await deleteFromFirestore('customer_licenses', `lic_${currentId}`);
