document.addEventListener('DOMContentLoaded', () => {
    ...
    async function init() {
        ...
        if (currentIdParam === 'new') {
            ...
            try {
                const docId = `lt_${id}`;
                const data = await getDocFromFirestore('license_types', docId);
                if (data) {
                    currentData = data;
                    loadData(data);
    ...
    function loadData(licenseType) {
        ...
        sortOrder.value = licenseType.sort_order || '';
    ...
    async function deleteData() {
        ...
        try {
            // Optimize: Use limit(1) to check existence instead of fetching all
            // Compat Syntax
        ...
        try {
            await deleteFromFirestore('license_types', `lt_${oldId}`);
