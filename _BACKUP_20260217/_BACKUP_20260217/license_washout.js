document.addEventListener('DOMContentLoaded', () => {
    // --- Selectors ---
    const washoutListBody = document.getElementById('washout-list-body');
    ...
    const filterRemarks = document.getElementById('filter-remarks');
    ...
    async function init() {
        try {
            console.log('Initializing License Washout List...');
    ...
    function renderFiscalMonthOptions() {
        ...
        // 「すべて」はデータ量削減のため一旦除外、または警告付きで実装も可だが、
        // 今回はQuota考慮し、月指定を原則とする運用に誘導。
        // const allOpt = document.createElement('option');
        // allOpt.value = 'all';
        // allOpt.textContent = 'すべて';
        // filterFiscalMonth.appendChild(allOpt);
    }
    ...
    async function searchData() {
        ...
        const remVal = filterRemarks.value.toLowerCase();
        ...
        try {
            ...
            // 4. クライアントサイドフィルタ (種別、担当者、備考)
            // サーバーサイドでやりすぎるとインデックス地獄になるため、残りはクライアントで。
            const finalData = joinedData.filter(item => {
                ...
                if (remVal !== "" && !String(item.customer.remarks || '').toLowerCase().includes(remVal)) return false;
                return true;
            });
    ...
    function renderTable(data, fmVal) {
        ...
        if (data.length === 0) {
            washoutListBody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding: 20px;">該当するデータはありません。</td></tr>';
            return;
        }
        ...
        data.forEach(item => {
            ...
            row.addEventListener('click', () => {
                // 許認可詳細画面へ遷移
                window.location.href = `license_detail.html?id=${item.license.license_id}`;
            });
            ...
            row.innerHTML = `
                ...
                <td>${expiry}</td>
                <td style="font-size: 0.85rem; color: #64748b;">${item.customer.remarks || '-'}</td>
            `;
    ...
    // 備考検索はEnterキーまたはフォーカスアウトで実行（連打防止）
    filterRemarks.addEventListener('change', searchData);
